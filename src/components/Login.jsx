...
import { DEMO_ACCOUNTS, loginDemo } from "../lib/demoAuth";
...
export const Login = () => {
  ...
  const handleDemoLogin = async (account) => {
    try {
      ...
      setError("Error con demo: " + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-surface py-12 px-4 sm:px-6 lg:px-8 hero-pattern">
      <div className="max-w-md w-full space-y-6">
        ...
        {/* Demo Accounts Card */}
        <div className="bg-surface-container-lowest p-6 rounded-xl border border-secondary/25 shadow-level-1">
          <div className="flex items-center gap-2 mb-4">
            <span className="material-symbols-outlined text-secondary text-xl">
              science
            </span>
            <span className="text-label-md font-label-md text-on-surface uppercase tracking-wider">
              Acceso Demo
            </span>
          </div>
          <p className="text-body-sm text-on-surface-variant mb-4">
            Prueba la plataforma sin registrarte. Las cuentas demo se crean
            automáticamente al primer click.
          </p>
          <div className="grid grid-cols-2 gap-3">
            {DEMO_ACCOUNTS.map((account) => (
              <button
                ...
                disabled={loading}
                className="flex flex-col items-center gap-2 p-4 rounded-xl border border-outline-variant/30 bg-surface-container-low hover:bg-surface-container hover:border-secondary/40 transition-all disabled:opacity-50 active:scale-[0.97]"
              >
                ...
                <span className="text-[11px] text-on-surface-variant font-mono text-center leading-tight">
                  {account.email}
                </span>
              </button>
            ))}
        ...
        <div className="bg-surface-container-lowest p-8 rounded-xl border border-outline-variant/30 shadow-level-1">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex justify-center py-3 px-4 bg-secondary text-on-secondary rounded-lg font-label-md text-label-md hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
